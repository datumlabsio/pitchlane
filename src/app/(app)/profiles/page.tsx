...
import { listProfileStatsGrouped } from '@/domain/profile-stats/repository';
...
export default async function ProfilesPage({ searchParams }: Props) {
  const [sp, profiles, projectsByAccount, statsByAccount] = await Promise.all([
    searchParams ?? Promise.resolve({}),
    listEditableProfiles(),
    listProjectsByAccount(),
    listProfileStatsGrouped(),
  ]);
  ...
  return (
    <div className="space-y-6">
      ...
      />
      <ProfileWorkbench profiles={profiles} selectedProfile={selectedProfile} projectsByAccount={projectsByAccount} statsByAccount={statsByAccount} />
    </div>
  );
}
